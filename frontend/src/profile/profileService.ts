...
/**
 * Interface for geo-location detection response
 */
...
export const profileService = {
  ...
  /**
   * Detect user's location and language based on IP address
   * @returns Detected location and language data
   */
  async detectLocation(): Promise<GeoLocationData> {
    try {
      const response = await profileClient.get('/profile/detect-location');
      return response.data;
    } catch (error) {
      console.error('Error detecting location:', error);
      // Return default empty values if detection fails
      return {
        country: undefined,
        language: 'en',
  ...
  /**
   * Complete user profile with automatic location detection
   * @param data Base profile data
   * @param useDetectedLocation Whether to use detected location data
   * @returns Completed user profile
   */
  async completeProfileWithLocation(
    ...
    try {
      ...
      // Try to detect location if requested and needed fields are missing
      if (useDetectedLocation && (!data.country || !data.language)) {
        try {
          const locationData = await this.detectLocation();
          
          // Only use detected data for fields not explicitly provided
          profileData = {
            ...data,
            country: data.country || locationData.country,
            city: data.city || locationData.city,
            timezone: data.timezone || locationData.timezone,
            language: data.language || locationData.language
          };
        } catch (locError) {
          console.error('Failed to detect location:', locError);
          // Continue with original data if detection fails
        }
      }
      
      // Complete the profile with the enhanced data
      return await this.completeUserProfile(profileData);
    } catch (error) {
      console.error('Error completing profile with location:', error);
      throw error;
    }
  }
};
