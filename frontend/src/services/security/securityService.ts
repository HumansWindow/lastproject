...
import { deviceFingerprint } from "./protection/deviceFingerprint";
...
class SecurityService {
  ...
  private readonly completedFactors: Set<AuthFactor> = new Set();
  ...
  public async getDeviceFingerprint(): Promise<string> {
    try {
      return await deviceFingerprint.getDeviceFingerprint();
    } catch (err) {
  ...
  public async evaluateRisk(action: string, context: Record<string, any> = {}): Promise<number> {
    // Base risk score
    let riskScore = 10; // Start with a base risk level
    
    // Check if device is known
    const isKnown = await this.isDeviceTrusted();
    if (!isKnown) {
      riskScore += 20; // Unknown devices have higher risk
    }
  ...
  public async recordEvent(
    ...
    try {
      ...
      console.warn('Failed to add device fingerprint to security event:', e);
    }
  ...
  public async trustCurrentDevice(): Promise<string> {
    const fingerprint = await deviceFingerprint.getDeviceFingerprint();
    // Store in localStorage to mark as trusted
    localStorage.setItem('trustedDevice', fingerprint);
    return fingerprint;
  }
  ...
  public async isDeviceTrusted(): Promise<boolean> {
    ...
    const trustedFingerprint = localStorage.getItem('trustedDevice');
    return trustedFingerprint === currentFingerprint;
  }
  ...
  public createSecureApiClient() {
    ...
    apiClient.interceptors.request.use(async (config: InternalAxiosRequestConfig) => {
      return await securityService.enhanceRequest(config);
    });
