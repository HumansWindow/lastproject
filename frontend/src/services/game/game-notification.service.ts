...
class GameNotificationService {
  ...
  public async getNotifications(
    ...
    try {
      const response = await api.get('/game/notifications', {
  ...
  public async markAsRead(notificationId: string): Promise<boolean> {
    try {
      await api.post(`/game/notifications/${notificationId}/read`);
  ...
  private async fetchUnreadCount(): Promise<void> {
    ...
    try {
      const response = await api.get('/game/notifications/count');
