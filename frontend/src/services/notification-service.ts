...
// Import the exact NotificationEvent interface from realtime-service
import { NotificationEvent as RealtimeNotificationEvent } from './api/realtime-service';
import { NotificationEvent as ApiNotificationEvent } from '../types/api-types';
...
class NotificationService {
  ...
  public initialize(): void {
    ...
    if (realtimeService) {
      // Create a type-compatible callback by using a type assertion
      // This works because we handle any missing properties in handleNotification
      this.unsubscribe = realtimeService.subscribeToNotifications(
        ((data: any) => this.handleNotification(data as RealtimeNotificationEvent)) as 
        (data: ApiNotificationEvent) => void
      );
    }
