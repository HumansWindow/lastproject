...
export class MonitoringApiClient {
  ...
  constructor(client = apiClient) {
    // Instead of passing the ApiClient directly, get its underlying AxiosInstance 
    // to avoid type compatibility issues
    this.client = axios.create({
      ...(client.getAxiosInstance?.() ? client.getAxiosInstance().defaults : client.defaults)
    });
    
    // Copy interceptors from original client
    const axiosInstance = client.getAxiosInstance?.() || client;
