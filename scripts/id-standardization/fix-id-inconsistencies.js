...
// Directories to explicitly skip to avoid symlink loops
const SKIP_DIRS = ['node_modules', 'dist', '.git', 'coverage', 'test'];
...
function findTsFiles() {
  const result = [];
  // Keep track of visited directories to avoid cycles in symbolic links
  const visitedDirs = new Set();
  
  function walk(dir) {
    // Skip if this directory has already been visited (prevents circular symlinks)
    if (visitedDirs.has(dir)) {
      return;
    }
    
    // Track this directory as visited
    visitedDirs.add(dir);
    ...
    for (const file of files) {
      ...
      try {
        const stat = fs.lstatSync(filePath); // Use lstatSync to detect symlinks
        ...
        // Skip files with permission issues or other errors
        if (VERBOSE) {
          console.log(`Skipping file due to error: ${filePath}`, error.code || error.message);
        }
        continue;
      }
    }
...
function findEntityFiles() {
  try {
    return findTsFiles().filter(file => {
      try {
        return file.includes('entity.ts') || 
 ...
 */
function standardizeColumnName(property) {
 ...
 */
function createReport(allIssues) {
  const reportData = {
...
`);
  ...
  if (DRY_RUN) {
    ...
    console.log('  node fix-id-inconsistencies.js --fix');
  }
}
