...
# Set variables
BACKEND_DIR="./backend"
DB_NAME="alive_db" # Change this to your database name if different
TIMESTAMP=$(date +"%Y%m%d%H%M%S")

# Create logs directory first, before any logging happens
mkdir -p ./logs
LOG_DIR="./logs"
...
# Using the JavaScript version instead of TypeScript to avoid type errors
node fix-id-inconsistencies.js --dry-run --verbose
if [ $? -ne 0 ]; then
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
  ...
  # Here we use your specific database name: Alive-Db
  PGPASSWORD="$DB_PASSWORD" psql -U "$DB_USER" -h localhost -d "Alive-Db" -f "$BACKEND_DIR/standardize-id-fields.sql" > logs/sql_execution_${TIMESTAMP}.log 2>&1
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
  ...
  # Using the JavaScript version instead of TypeScript to avoid type errors
  node fix-id-inconsistencies.js --fix
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
  ...
  # Using the JavaScript version instead of TypeScript to avoid type errors
  node fix-id-inconsistencies.js --verbose
