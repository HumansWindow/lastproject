...
const entityFixReplacements = [
  ...
  {
    pattern: /GameModuleEntity/g,
    replacement: 'GameModule'
  }
];
...
const repositoryFixReplacements = [
  ...
  {
    pattern: /@InjectRepository\(GameModuleEntity\)/g, 
    replacement: '@InjectRepository(GameModule)'
  }
];
...
const returnTypeFixes = [
  {
    pattern: /\): Promise<GameNotificationTemplateEntity \| null>/g,
    replacement: '): Promise<GameNotificationTemplate | null>'
  },
  {
    pattern: /module\?: GameModuleEntity;/g,
    replacement: 'module?: GameModule;'
  },
  {
    pattern: /mapTemplateToDto\(template: GameNotificationTemplateEntity\)/g,
    replacement: 'mapTemplateToDto(template: GameNotificationTemplate)'
  },
  {
    pattern: /mapScheduleToDto\(schedule: ModuleNotificationScheduleEntity\)/g,
    replacement: 'mapScheduleToDto(schedule: ModuleNotificationSchedule)'
  },
  {
    pattern: /mapUserNotificationToDto\(notification: UserNotificationEntity\)/g,
    replacement: 'mapUserNotificationToDto(notification: UserNotification)'
  }
];

// Fix DTO property mappings
const dtoMappingFixes = [
  // Fix NotificationTemplateDto
  {
    pattern: /{\s*id: template\.id,\s*moduleId: template\.moduleId,\s*moduleName: template\.module\?\.title,[\s\S]*?}\s*}/g,
    replacement: (match) => {
      return match
        .replace(/moduleName: template\.module\?\.title,/g, '');
    }
  },
  // Fix NotificationScheduleDto
  {
    pattern: /{\s*id: schedule\.id,\s*moduleId: schedule\.moduleId,\s*moduleName: schedule\.module\?\.title,[\s\S]*?}\s*}/g,
    replacement: (match) => {
      return match
        .replace(/moduleName: schedule\.module\?\.title,/g, '');
    }
  },
  // Fix UserNotificationDto
  {
    pattern: /{\s*id: notification\.id,\s*userId: notification\.userId,\s*moduleId: notification\.moduleId,\s*moduleName: notification\.module\?\.title,[\s\S]*?}\s*}/g,
    replacement: (match) => {
      return match
        .replace(/moduleName: notification\.module\?\.title,/g, '');
    }
  },
];
...
// 4. Fix return type declarations
returnTypeFixes.forEach(fix => {
  content = content.replace(fix.pattern, fix.replacement);
});
...
content = content.replace(
  ...
  match => {
    return match
      ...
      .replace(/GameModuleEntity/g, 'GameModule');
  }
);

// 7. Update repository types in constructor
content = content.replace(
