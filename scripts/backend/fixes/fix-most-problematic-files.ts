...
// Define TypeScript types even though we're using CommonJS
...
function runTsScript(scriptPath: string): Promise<string> {
  return new Promise((resolve, reject) => {
    // Use ts-node to execute the script
    const process = childProcess.exec(`npx ts-node ${scriptPath}`, (error: ExecError | null, stdout: string, stderr: string) => {
      if (error) {
