...
if [ -f "${BACKEND_DIR}/dist/main.js" ]; then
    ...
    cd "${BACKEND_DIR}" && node --expose-gc --max-old-space-size=512 dist/main.js 2>&1 | tee "$LOG_FILE"
else
    ...
    cd "${BACKEND_DIR}" && NODE_OPTIONS="--max-old-space-size=512" npm run start:dev 2>&1 | tee "$LOG_FILE"
fi
